const CACHE='vencivo-v3';
const APP=['/','/index.html','/ia.html','/ia-v2.html','/whatsapp-config.html','/manifest.json','/icon.svg','/implant-flow.js'];
...
async function networkHtml(request){
  try{
    ...
    if(path.endsWith('/ia-v2.html')||path.endsWith('/whatsapp-config.html')){
      ...
      if(!text.includes('/implant-flow.js')){
        const injected=text.replace('</body>','<script type="module" src="/implant-flow.js"></script></body>');
