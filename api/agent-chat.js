...
export default async function handler(req,res){
 ...
 try{
  ...
  const rows=await db(`agents?public_id=eq.${encodeURIComponent(id)}&status=in.(demo,active)&select=id,public_id,company_name,agent_name,segment,system_prompt&limit=1`);const agent=rows?.[0];if(!agent)return out(res,req,404,{error:'Agente não encontrado ou indisponível.'});
