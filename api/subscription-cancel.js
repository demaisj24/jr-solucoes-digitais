...
function withinSevenDays(date){if(!date)return false;const t=new Date(date).getTime();return Number.isFinite(t)&&Date.now()-t<=7*86400000}
...
export default async function handler(req,res){
 ...
 try{
  ...
  let refund=false,refundPending=false,refundMessage='';
  // Localiza uma cobrança já recebida/confirmada da assinatura antes de removê-la.
  if(sub.provider_subscription_id){
   const paid=await asaas(`/subscriptions/${encodeURIComponent(sub.provider_subscription_id)}/payments?status=RECEIVED`);
   const confirmed=paid.ok?paid.data:null;
   const payments=Array.isArray(confirmed?.data)?confirmed.data:[];
   const payment=payments.find(p=>withinSevenDays(p.paymentDate||p.confirmedDate||p.dateCreated));
   if(payment?.id&&withinSevenDays(sub.trial_started_at)){
    ...
    if(rr.ok)refund=true;else{refundPending=true;refundMessage='O cancelamento foi realizado, mas o estorno automático não foi concluído. A solicitação ficará pendente para tratamento.';console.error('Asaas refund',rr.status,rr.data)}
   }
  ...
  if(refundPending)return json(res,200,{ok:true,status:'canceled',refund_pending:true,message:refundMessage});
