...
async function db(path,options={}){if(!SERVICE_ROLE_KEY)throw new Error('Supabase não configurado no servidor.');const controller=new AbortController();const timeout=setTimeout(()=>controller.abort(),SUPABASE_TIMEOUT_MS);try{const response=await fetch(`${SUPABASE_URL}/rest/v1/${path}`,{...options,signal:controller.signal,headers:{apikey:SERVICE_ROLE_KEY,Authorization:`Bearer ${SERVICE_ROLE_KEY}`,'Content-Type':'application/json',Prefer:options.method==='POST'?'return=representation':undefined,...(options.headers||{})}});const payload=await response.json().catch(()=>null);if(!response.ok){console.error('Supabase error',response.status,payload);throw new Error('Falha ao persistir agente.')}return payload}catch(error){if(error?.name==='AbortError'){console.error('Supabase timeout:',path);throw new Error('O banco de dados demorou mais que o esperado. Tente novamente.')}throw error}finally{clearTimeout(timeout)}}
async function authUser(req){const authorization=String(req.headers.authorization||'');if(!authorization.startsWith('Bearer ')||!SERVICE_ROLE_KEY)return null;const controller=new AbortController();const timeout=setTimeout(()=>controller.abort(),SUPABASE_TIMEOUT_MS);try{const response=await fetch(`${SUPABASE_URL}/auth/v1/user`,{headers:{apikey:SERVICE_ROLE_KEY,Authorization:authorization},signal:controller.signal});if(!response.ok)return null;const user=await response.json();return user?.id?user:null}catch(error){if(error?.name==='AbortError')console.error('Auth timeout');return null}finally{clearTimeout(timeout)}}
export default async function handler(req,res){
 ...
 try{
  ...
  if(req.method==='POST'){
   ...
   if(!saved)throw new Error('Agente não foi retornado pelo banco.');
   const knowledge=clean(b.knowledge_text,80000);if(knowledge){await db('agent_knowledge',{method:'POST',body:JSON.stringify({agent_id:saved.id,file_name:clean(b.knowledge_file_name,160)||'base-de-conhecimento.txt',content:knowledge,content_type:'text/plain',size_bytes:knowledge.length})})}
 ...
 }catch(error){console.error('Agents API:',error);return json(res,req,500,{error:error?.message||'Não foi possível processar o agente agora.'})}
}
