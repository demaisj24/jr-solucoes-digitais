...
export default async function handler(req,res){
 ...
 try{
  ...
  const data=await db(`agents?public_id=eq.${encodeURIComponent(id)}&select=public_id,company_name,agent_name,segment,whatsapp,city_region,services,business_hours,capabilities,personality,objective,status,created_at&limit=1`,{method:'GET'});if(!data?.length)return json(res,req,404,{error:'Agente não encontrado.'});
