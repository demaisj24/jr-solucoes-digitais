...
Garantir que o sistema possa ser reconstruído mesmo em caso de:

- perda do repositório local;
- erro humano em produção;
- exclusão/corrupção do banco;
...
Conclusão: o volume atual permite backup lógico frequente com custo operacional baixo. O desenho deve, porém, continuar válido quando o volume crescer.
...
Risco HIGH de continuidade:

- o histórico completo das migrations não está versionado na `main`;
- portanto o GitHub, sozinho, ainda não é suficiente para reconstruir integralmente o schema do Supabase.
...
- policies RLS;
- grants relevantes;
- dados necessários da aplicação.
...
O procedimento de restore deve validar:

- usuários;
- identidades;
- relações entre `auth.users` e `public.profiles`;
- sessões não precisam necessariamente ser preservadas como requisito funcional, mas a decisão deve ser explícita.
...
Manter inventário sem valores reais de:
...
## 8. Destino do backup — decisão pendente

O backup não deve ficar apenas:

- na mesma máquina de desenvolvimento;
- no mesmo projeto Supabase;
- no mesmo repositório público;
- em artifact temporário sem política de retenção.

Antes da automação final, escolher um destino privado off-site com:
...
Exemplos possíveis a avaliar: storage S3 compatível, bucket privado dedicado ou outro provedor independente. Não escolher/implantar sem validar credenciais, custo e restauração.
...
Impacto: projeto Free depende de exportação própria para proteção off-site.
...
1. escolher destino off-site do backup;
2. definir método seguro de conexão ao Postgres para `pg_dump`/Supabase CLI;
3. criar workflow de backup em branch isolada;
4. executar primeiro backup real;
5. validar checksum;
6. executar restore em ambiente separado;
7. atualizar `OPERACAO-MANUTENCAO-E-BACKUP.md` com procedimento para leigo;
8. só então marcar BACKUP/DISASTER RECOVERY como concluído.
