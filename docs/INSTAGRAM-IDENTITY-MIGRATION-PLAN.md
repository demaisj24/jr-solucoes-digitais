...
## 5. Schema de `instagram_webhook_events` (desenho v2, não aplicar ainda)

`docs/sql/instagram-webhook-events.sql`. Campos: `id`, `provider_event_id` (`UNIQUE`, é o `dedupeKeyForEntry()` do INST-04/04A — limitação conhecida: dedup por `entry` inteiro, não por mensagem/comentário individual), `instagram_user_id`, `agent_id` (nullable), `event_type`, **`status`** (`received`/`processing`/`processed`/`failed` — ciclo de vida explícito, novo em relação à v1), `processed_at` (setado só quando `status='processed'`), **`payload jsonb` com `CHECK` que exige o schema `{entry_id, time, item_count, item_types}`** (não mais só um comentário de intenção — agora o próprio banco rejeita qualquer payload fora desse formato, incluindo o payload bruto da Meta). RLS ligada **+ `REVOKE ALL` explícito** de `authenticated`/`anon` (não depender de RLS-sem-policy implícito — ver seção 5a). Não reaproveita `billing_events`.

**Payload — schema concreto (v2, substitui a nota "mínimo/seguro" da v1):** `{ "entry_id": text, "time": number, "item_count": integer, "item_types": text[] }`. Nunca: texto de mensagem/comentário, nome de remetente/comentarista, ids individuais (já estão no hash de `provider_event_id`), token/secret, **mídia bruta (áudio/imagem/vídeo/URL de mídia)** — retrieval de mídia é responsabilidade de uma etapa própria futura, com sua própria política de retenção, nunca guardada nesta tabela.

**Retenção:** 90 dias a partir de `created_at`, avaliada e documentada na própria migration — não implementada (precisa de `pg_cron`, **confirmado não instalado neste projeto** via `list_extensions`).

## 5a. RLS — por que `REVOKE ALL` explícito, e não só `RLS sem policy`

A v1 confiava em "RLS habilitada, sem nenhuma policy = nega tudo por padrão" (mesmo padrão de `billing_events`). Isso funciona na prática — testei empiricamente contra `billing_events`: `has_table_privilege('authenticated', 'public.billing_events', 'SELECT')` = `false`. Mas encontrei em `pg_default_acl` que este projeto tem ACLs padrão que concedem privilégio de tabela completo a `anon`/`authenticated`/`service_role` em tabelas novas — o mesmo tipo de concessão implícita que causou o FAIL corrigido em `instagram_connections` (lá, havia policies de RLS que dependiam desse grant; aqui não há policy nenhuma, então a exposição prática é nula mesmo com o grant implícito presente). Ainda assim, para não depender de inferência sobre comportamento de ACL padrão — e deixar a intenção auditável direto no SQL — a v2 adiciona `REVOKE ALL ON instagram_webhook_events FROM authenticated, anon;` explícito.

## 5b. `pgmq` — situação real, não presumida

`list_extensions` confirma `pgmq` disponível (`default_version 1.5.1`) mas **`installed_version: null`** — não instalado. `select nspname from pg_namespace where nspname='pgmq'` retornou vazio, confirmando que nenhuma função `pgmq.*` existe neste banco hoje. A análise de atomicidade (registro do evento + publicação na fila + claim + retry), documentada no fim de `docs/sql/instagram-webhook-events.sql`, é baseada na documentação oficial (`supabase.com/docs/guides/queues/pgmq`), não testada neste projeto. Antes de qualquer implementação de worker: `create extension pgmq;` precisa ser uma migration própria, aprovada separadamente — fora do escopo do INST-05B.
