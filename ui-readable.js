...
(function(){
  ...
  function normalizeSend(){
    ...
    if(send){send.textContent='ENVIAR';send.setAttribute('aria-label','Enviar mensagem');}
  }
  ...
  async function startImplant(e){
    ...
    try{
      b.textContent='PREPARANDO PAGAMENTO...';b.style.pointerEvents='none';
      ...
      b.textContent='QUERO IMPLANTAR';b.style.pointerEvents='';toast(err.message||'Não foi possível iniciar a implantação agora.');
    }
  }

  function toast(text){let el=document.getElementById('vencivoReadableToast');if(!el){el=document.createElement('div');el.id='vencivoReadableToast';Object.assign(el.style,{position:'fixed',left:'50%',bottom:'20px',transform:'translateX(-50%)',zIndex:'99999',padding:'12px 16px',borderRadius:'12px',background:'#111a2a',color:'#fff',border:'1px solid #ffffff22',boxShadow:'0 15px 50px #0008',font:'600 14px system-ui',maxWidth:'90vw',textAlign:'center'});document.body.appendChild(el)}el.textContent=text;el.style.display='block';clearTimeout(window._vru);window._vru=setTimeout(()=>el.style.display='none',2800)}

  function normalizeImplant(){
    const b=document.getElementById('implant');
    if(!b)return;
    b.textContent='QUERO IMPLANTAR';
    b.removeAttribute('href');b.removeAttribute('target');
